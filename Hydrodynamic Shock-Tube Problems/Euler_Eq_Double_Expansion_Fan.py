"""
Forward Problem for 1-D Euler Equations for Compressible Flow: Double Expansion Fan
...
In this code we solve for rho, u, p  from the following equations:
                              ...
                              (rho,u,p)_t=0 = (1.0,-1.0,0.4) 0 <= x < 0.5
                              (rho,u,p)_t=0 = (1.0,1.0,0.4) 0.5 <= x <=1
...
def IC(x):
    ...
    p_init = 0.4*np.ones((x.shape[0]))                                          # p - initial condition
...
def main():
    ...
    epochs = 40165                                                        # Number of iterations
    ...
    x = np.linspace(-2.6175, 2.5,num_x)                                   # Partitioned spatial axis
    ...
    scipy.io.savemat('Double_Expansion.mat', {'x': x, 't': t,'rho': u_pred[:,0],
                                                              'u': u_pred[:,2],
                                                              'p': u_pred[:,1]})
